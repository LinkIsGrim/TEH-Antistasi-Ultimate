...
private _spawnPos = _markerPos getPos [_spawnDistance, _spawnDir]; //prepare random landing?
private _midWaypoint = _markerPos getPos [300, _spawnDir];

_potentials = outposts + airportsX + milbases + resourcesX + factories + seaports;
_potentials = _potentials select { (spawner getVariable _x == 2) and (sidesX getVariable [_x, sideUnknown] == Occupants) and (_markerPos distance2d (getMarkerPos _x) < 2000) };
